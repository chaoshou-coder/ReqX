...
class RequirementExcavationSkill(BaseTool):
    ...
    def _normalize_and_validate(self, data: Any) -> tuple[dict[str, Any] | None, list[str]]:
        ...
        if not isinstance(demand_id, str) or not demand_id.strip():
            errors.append("demand_id must be a non-empty string")
        ...
        if not isinstance(ps, list):
            errors.append("proposed_solutions must be a list")
        else:
            for i, item in enumerate(ps):
                if not isinstance(item, dict):
                    errors.append(f"proposed_solutions[{i}] must be an object")
                    continue
                desc = item.get("description")
                if not isinstance(desc, str) or not desc.strip():
                    errors.append(f"proposed_solutions[{i}].description must be a non-empty string")
                for lk in ["pros", "cons"]:
                    lv = item.get(lk)
                    if not isinstance(lv, list) or any(not isinstance(x, str) for x in lv):
                        errors.append(f"proposed_solutions[{i}].{lk} must be a list of strings")
                risks = item.get("optimization_risks")
                if not isinstance(risks, list):
                    errors.append(f"proposed_solutions[{i}].optimization_risks must be a list")
                else:
                    for j, r in enumerate(risks):
                        if not isinstance(r, dict):
                            errors.append(f"proposed_solutions[{i}].optimization_risks[{j}] must be an object")
                            continue
                        desc = r.get("desc")
                        if not isinstance(desc, str) or not desc.strip():
                            errors.append(
                                f"proposed_solutions[{i}].optimization_risks[{j}].desc must be a non-empty string"
                            )
                        mitigation = r.get("mitigation")
                        if not isinstance(mitigation, str) or not mitigation.strip():
                            errors.append(
                                f"proposed_solutions[{i}].optimization_risks[{j}].mitigation must be a non-empty string"
                            )
        ...
        if isinstance(selected_solution, str):
            if selected_solution.strip():
                ...
                errors.append("selected_solution must be an object")
        elif isinstance(selected_solution, dict):
            name = selected_solution.get("name")
            reason = selected_solution.get("reason")
            if not isinstance(name, str) or not name.strip():
                errors.append("selected_solution.name must be a non-empty string")
            if not isinstance(reason, str) or not reason.strip():
                errors.append("selected_solution.reason must be a non-empty string")
    ...
    def _run(self, surface_problem: str) -> str:
        try:
            ...
            return self._error_yaml(
                code="llm_config_error", message="LLM 配置或初始化失败", details={"exception": redact_secrets(str(e))}
            )
        try:
        ...
        try:
            ...
            details: dict[str, Any] = {
                "parse_error": redact_secrets(str(e)),
        ...
        if normalized is None:
            details: dict[str, Any] = {"schema_errors": errors, "raw_output_len": len(raw_text)}
            if include_raw:
                details["raw_output_preview"] = raw_preview
                details["raw_output_truncated"] = raw_truncated
            if surface_problem_truncated:
