...
def load_llm_config(path: str | os.PathLike[str] | None = None, *, strict: bool = False) -> LLMConfig:
    ...
    if not config_path.exists():
        if strict:
            raise RuntimeError(f"缺少 LLM 配置文件：{config_path}（请先运行 letsgo.py 生成 llm.yaml）")
...
def get_llm(*, config_path: str | os.PathLike[str] | None = None, strict: bool = True, **overrides: Any):
    ...
    resolved_config_path = Path(config_path) if config_path else _default_config_path()
    ...
    if provider == "openai" and cfg.base_url:
